...
from database import supabase_request, find_or_create_customer
...
async def create_order(request: Request, background_tasks: BackgroundTasks):
    ...
    try:
        ...
        try:
            ...
            raise HTTPException(status_code=400, detail=f"Failed to parse request data: {e}")
        ...
        if not data["customer_name"].strip():
            print(f"❌ Empty customer name: '{data['customer_name']}'")
            raise HTTPException(status_code=400, detail="Customer name cannot be empty")
        
        # More flexible phone validation
        phone = data["customer_phone"].strip()
        if not phone or len(phone) < 9:
            print(f"❌ Invalid phone: '{phone}' (length: {len(phone)})")
            raise HTTPException(status_code=400, detail="Phone number must be at least 9 digits")
        
        print(f"📱 Phone validation passed: {phone}")
        
        if not data["items"]:
            print(f"❌ No items in order")
            raise HTTPException(status_code=400, detail="Order must contain at least one item")
        
        if float(data["total_amount"]) <= 0:
            print(f"❌ Invalid total amount: {data['total_amount']}")
            raise HTTPException(status_code=400, detail="Total amount must be positive")
        
        if data["order_type"] not in ["pickup", "delivery"]:
            print(f"❌ Invalid order type: {data['order_type']}")
            raise HTTPException(status_code=400, detail="Order type must be 'pickup' or 'delivery'")
            
        print(f"✅ All validations passed!")
        
        # Create or find customer
        print(f"👤 Creating/finding customer: {data['customer_name']}")
        try:
            customer_id = await find_or_create_customer(
                name=data["customer_name"],
                phone=data["customer_phone"],
                platform=data.get("platform", "WEB"),
                platform_user_id=data.get("platform_user_id", data["customer_phone"])
            )
            print(f"✅ Customer ID: {customer_id}")
        except HTTPException as e:
            print(f"❌ Customer creation failed: {e.detail}")
            raise e
        except Exception as e:
            print(f"❌ Unexpected customer error: {e}")
        ...
        thailand_tz = timezone(timedelta(hours=7))  # UTC+7 for Bangkok
        ...
        for item in data["items"]:
            # Handle menu_id - if it's not a valid UUID, set to None
            menu_id = item.get("id")
            if menu_id:
                # Check if it looks like a UUID (36 chars with dashes)
                import re
                uuid_pattern = r'^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$'
                if not re.match(uuid_pattern, str(menu_id), re.IGNORECASE):
                    menu_id = None  # Invalid UUID format
                    print(f"⚠️ Invalid menu_id '{item.get('id')}' - setting to None")
                
            item_data = {
                "order_id": order["id"],
                "menu_id": menu_id,  # Can be None for custom items
                "menu_name": item["name"],
